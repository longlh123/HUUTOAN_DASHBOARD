...
class SalesPerformanceService
{
    ...
    // Department tinh doanh so theo hoa don ERP thay vi CRM Won quote (xem docs/sales-crm-backlog-tracking.md muc 3).
    // Da xac nhan string chinh xac qua tinker: CRM luu "NNC" va "SS" (khong phai "B2C"/"SEV").
    private const ERP_SOURCED_DEPARTMENTS = ['NNC', 'SS'];

    public function __construct(
        private CrmApiService $api,
        private ErpSalesAttributionService $erp,
    ) {}
    ...
    public function byRep(Carbon $from, Carbon $to, ?string $territory = null, ?string $department = null): Collection
    {
        ...
        return $allOwnerIds->map(function ($ownerId) use ($wonByOwner, $lostByOwner, $kpiByRep) {
            ...
            $avgDays = $ownerOppRows->avg(fn ($q) => max(0, Carbon::parse($q['createdon'])->diffInDays(Carbon::parse($q['closedon'])))
            );
    ...
    public function byTeam(Carbon $from, Carbon $to, ?string $territory = null, ?string $department = null): Collection
    {
        ...
        return $allTeamIds->map(function ($teamId) use ($wonByTeam, $lostByTeam, $users, $kpiByTeam) {
            ...
            $avgDays = $memberOppRows->avg(fn ($q) => max(0, Carbon::parse($q['createdon'])->diffInDays(Carbon::parse($q['closedon'])))
            );
    ...
    public function weeklyDeals(int $weekOffset = 0, ?string $territory = null, ?string $department = null): array
    {
        ...
        $quotes = $this->wonDealsFor($monday, $sunday, $territory, $department, 300);
    ...
    public function dailyReport(?string $territory = null, ?string $date = null): array
    {
        ...
        $weekDeals = $this->calcOppValue(
            $this->wonDealsFor($weekStart, $weekEnd, $territory, null, 300)
        );
    ...
    private function resolveQuoteValues(Collection $quotes, int $ttl = 1800): Collection
    {
        ...
        $valueByQuote = Cache::lock("lock:{$cacheKey}", 120)->block(120, fn () => cache()->remember($cacheKey, $ttl, function () use ($quoteIds, $ttl) {
            $detailsByQuote = $this->fetchQuoteDetails($quoteIds, $ttl);

            return $detailsByQuote->map(fn ($lines) => $lines->sum(fn ($l) => (float) ($l['ab_total_amount_plus_ut_no_tax_calculated'] ?? 0)
    ...
    public function pipeline(?string $territory = null, ?string $department = null): array
    {
        ...
        $opps = $opps->filter(fn ($o) => isset($anyByOpp[$o['opportunityid']]) &&
        ...
        $currentQOpps = $opps->filter(fn ($o) => ! empty($o['estimatedclosedate']) &&
        ...
        $nextQOpps = $opps->filter(fn ($o) => ! empty($o['estimatedclosedate']) &&
        ...
        $next2QOpps = $opps->filter(fn ($o) => ! empty($o['estimatedclosedate']) &&
    ...
    private function fetchOpenOpportunities(): Collection
    {
        ...
        $data = cache()->remember('crm:pipeline_open_opportunities', 300, fn () => $this->api->getAll('opportunities', [
    ...
    private function fetchWonQuotesForOpps(array $oppIds): Collection
    {
        ...
        $all = cache()->remember('crm:pipeline_won_quotes', 300, fn () => $this->api->getAll('quotes', [
        ...
        return collect($all)
            ->filter(fn ($q) => ! empty($q['_opportunityid_value']) &&
    ...
    private function fetchAnyQuotesForOpps(array $oppIds): Collection
    {
        ...
        $all = cache()->remember('crm:pipeline_active_draft_quotes', 300, fn () => $this->api->getAll('quotes', [
    ...
    {
        return $year < now()->year ? Carbon::create($year, 12, 31)->endOfDay() : now()->copy()->endOfDay();
    }
    ...
    {
        if (! $territory || strtoupper($territory) === 'ALL') {
            return $deals;
        }

        $ownerIds = $this->fetchUsers()
            ->filter(fn ($u) => strtoupper($u['_ab_dim_territory_id_value@OData.Community.Display.V1.FormattedValue'] ?? '') === strtoupper($territory)
            )
            ->keys()
            ->toArray();
    ...
    // Nguoc lai filterByDepartment() — loai deal cua owner thuoc 1 trong cac department cho truoc.
    // Dung de loai NNC/SS ra khoi CRM deals truoc khi cong them deal ERP, tranh double-count.
    private function excludeByDepartment(Collection $deals, array $departments): Collection
    {
        $excludedOwnerIds = $this->fetchUsers()
            ->filter(fn ($u) => in_array($u['_ab_dim_department_id_value@OData.Community.Display.V1.FormattedValue'] ?? '', $departments, true))
            ->keys()
            ->toArray();

        return $deals->reject(fn ($d) => in_array($d['_ownerid_value'] ?? null, $excludedOwnerIds, true));
    }

    // Nguoc lai excludeByDepartment() — CHI giu deal cua owner thuoc 1 trong cac department cho truoc.
    // Dung de gioi han deal ERP trong view "ALL" chi con NNC/SS — erp->dealRows() tra ve MOI deal
    // no biet toi (ca SO lan OnAccount), khong tu loc department; neu khong gioi han o day, deal cua
    // owner B2B/OEM đi qua luong SO/OnAccount se bi cong THEM vao ben canh so CRM Won quote cua ho
    // (da co san trong crmOnly) — double-count, lam B2B tang bat thuong thay vi giu nguyen.
    private function includeByDepartments(Collection $deals, array $departments): Collection
    {
        $includedOwnerIds = $this->fetchUsers()
            ->filter(fn ($u) => in_array($u['_ab_dim_department_id_value@OData.Community.Display.V1.FormattedValue'] ?? '', $departments, true))
            ->keys()
            ->toArray();

        return $deals->filter(fn ($d) => in_array($d['_ownerid_value'] ?? null, $includedOwnerIds, true));
    }

    /**
     * Deal Won da resolve gia tri (resolved_value), da loc territory/department — nguon blend:
     * B2B/OEM/... van tu CRM Won quote, NNC/SS tu hoa don ERP (ErpSalesAttributionService).
     * Khi $department = null (view "ALL"), CRM phai loai owner thuoc NNC/SS ra truoc (da chuyen
     * sang ERP) roi moi cong them deal ERP vao, tranh tinh trung doanh so.
     */
    private function wonDealsFor(Carbon $from, Carbon $to, ?string $territory = null, ?string $department = null, ?int $ttl = null): Collection
    {
        if ($department && in_array($department, self::ERP_SOURCED_DEPARTMENTS, true)) {
            return $this->filterByTerritory(
                $this->filterByDepartment($this->erp->dealRows($from, $to), $department),
                $territory
            );
        }

        $crmDeals = $this->resolveQuoteValues(
            $this->filterByTerritory($this->filterByDepartment($this->fetchWonQuotes($from, $to), $department), $territory),
            $ttl ?? $this->ttlFor($to)
        );
        ...
        $crmOnly = $this->excludeByDepartment($crmDeals, self::ERP_SOURCED_DEPARTMENTS);
        $erpDeals = $this->filterByTerritory(
            $this->includeByDepartments($this->erp->dealRows($from, $to), self::ERP_SOURCED_DEPARTMENTS),
            $territory
        );
    ...
    public function allUsers(): Collection
    {
        ...
        return collect($data['value'] ?? [])
            ->filter(fn ($u) => str_ends_with(strtolower($u['internalemailaddress'] ?? ''), '@huutoan.com') &&
    ...
    private function fetchOpenOppsForQuality(): Collection
    {
        ...
        $data = cache()->remember('crm:quality_open_opportunities', 300, fn () => $this->api->getAll('opportunities', [
    ...
    public function quoteRequestTypeCrosstab(int $year, ?string $territory = null, ?string $department = null): array
    {
        ...
        usort($codeList, fn ($a, $b) => $a === '(Chua nhap)' ? 1 : ($b === '(Chua nhap)' ? -1 : strcmp($a, $b))
        );
