...
use App\Http\Controllers\Dashboard\KpiTeamController;
use App\Http\Controllers\Dashboard\OpportunitiesController;
use App\Http\Controllers\Dashboard\ProjectTrackingController;
use App\Http\Controllers\Dashboard\SalesController;
use App\Http\Controllers\Dashboard\UsersController;
use App\Http\Middleware\EnforceTerritory;
use App\Models\User;
...
// Dashboard
Route::middleware(['auth:sanctum', EnforceTerritory::class])
    ->prefix('dashboard')
    ->group(function () {
        ...
        Route::get('/sales/erp-reconciliation', [SalesController::class,      'erpReconciliation']);
        Route::get('/sales/projects', [ProjectTrackingController::class, 'index']);
        Route::get('/sales/projects/{code}', [ProjectTrackingController::class, 'show']);
        ...
        Route::get('/devices/inventory-stock-list', [DeviceController::class, 'inventoryStockList']);
        ...
        // Dat truoc /kpi/{id} — route wildcard 1 segment se nuot nham "company-target"/"team-targets"
        // neu khai bao sau.
        Route::get('/kpi/company-target', [KpiTeamController::class, 'companyTarget']);
        Route::put('/kpi/company-target', [KpiTeamController::class, 'saveCompanyTarget']);
        Route::get('/kpi/team-targets', [KpiTeamController::class, 'teamTargets']);
        Route::post('/kpi/team-targets', [KpiTeamController::class, 'storeTeamTarget']);
        Route::put('/kpi/team-targets/{id}', [KpiTeamController::class, 'updateTeamTarget']);
        Route::delete('/kpi/team-targets/{id}', [KpiTeamController::class, 'destroyTeamTarget']);
